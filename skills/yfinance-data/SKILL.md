---
name: yfinance-data
description: '[FROZEN] Financial/Trading skill frozen by user request.'
metadata:
  status: frozen
---
...
## Step 1: Check Funda AI First

**CRITICAL RULE:** `funda-data` is the primary skill for all market data. You must attempt to use `funda-data` first.
Only proceed with this `yfinance-data` skill if:
1. `funda-data` failed or returned an error.
2. The user explicitly asked for "Yahoo Finance" or "yfinance".
